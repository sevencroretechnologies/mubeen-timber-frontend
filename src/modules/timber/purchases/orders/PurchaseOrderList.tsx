...
const getAvailableActions = (status: PurchaseOrderStatus): ActionButton[] => {
    ...
    return [...baseActions, ...(statusActions[status] || [])].sort(
...
function ActionMenu({ actions, onAction, id }: ActionMenuProps) {
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent
                align='end'
                className='w-48 min-w-[180px] rounded-lg border-slate-200'
            >
                {actions.map((action) => {
                    ...
                    const variantClass = {
                        ...
                        invoice:
                            "text-blue-600 focus:text-blue-700 focus:bg-blue-50",
                    }[action.variant];
    ...
    return (
        <div className='space-y-4'>
            {[1, 2, 3].map((i) => (
                <Card
                    key={i}
                    className='rounded-xl shadow-sm border-none bg-white p-4'
                >
        ...
        <Card className='rounded-xl shadow-sm border border-slate-100 bg-white p-4 transition-all hover:shadow-md'>
            <div className='flex justify-between items-start mb-3'>
                <div className='space-y-1 min-w-0 flex-1'>
                    <h3 className='font-bold text-slate-900 text-base sm:text-lg uppercase tracking-tight truncate'>
                        {order.po_code}
                    </h3>
            ...
            <div className='mt-4 pt-3 border-t border-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-3'>
                <div className='flex items-center justify-between sm:justify-start gap-2 min-w-0'>
                    <div className='flex items-center gap-1 text-slate-500 min-w-0'>
                        <span className='text-[10px] font-black uppercase text-slate-300 shrink-0'>
                            W/H:
                        </span>
                        <span className='font-bold text-slate-600 text-[11px] sm:text-xs truncate max-w-[100px] sm:max-w-none'>
                    ...
                    <div className='sm:hidden'>
                        <ActionMenu
                            actions={
                                getActionGroups(order.status).remainingActions
                            }
                            onAction={(action) => {
                                switch (action.type) {
                                    case "view":
...
export default function PurchaseOrderList() {
    ...
    const handleSend = async (id: number) => {
        ...
        try {
            ...
            showAlert(
                "error",
                "Error",
                getErrorMessage(error, "Failed to send order"),
            );
        }
    };
    ...
    const handleCancel = async (id: number) => {
        const result = await showConfirmDialog(
            "Cancel Order",
            "Are you sure you want to cancel this purchase order? This will cancel the order.",
        );
        if (!result.isConfirmed) return;
        try {
    ...
    const columns: TableColumn<TimberPurchaseOrder>[] = [
        {
            name: "#",
            selector: (_row, index) =>
                (page - 1) * perPage + (index !== undefined ? index + 1 : 0),
            width: "60px",
        },
        {
            name: "PO Number",
        ...
        },
        {
            ...
            right: true,
            cell: (row) => (
                <span className='font-semibold'>{`₹${Number(row.total_amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`}</span>
            ),
            width: "140px",
        },
        {
            ...
            <div className='flex items-center justify-between gap-3'>
                <div className='min-w-0'>
                    ...
                    <p className='text-xs sm:text-sm text-slate-500'>
                        Manage purchase orders for timber procurement
                    </p>
                </div>
                <Button
                    size="sm"
                    onClick={() => navigate("/purchases/orders/create")}
                    className='bg-solarized-blue hover:bg-solarized-blue/90 shadow-sm shadow-solarized-blue/20 font-semibold text-xs sm:text-sm px-3 sm:px-4 h-8 sm:h-9 shrink-0'
                >
                    ...
                    >
                        ...
                        <div className='flex gap-3'>
                            ...
                            >
                                ...
                                <option value={PURCHASE_ORDER_STATUS.DRAFT}>
                                    Draft
                                </option>
                                <option value={PURCHASE_ORDER_STATUS.ORDERED}>
                                    Ordered
                                </option>
                                <option
                                    value={
                                ...
                                <option value={PURCHASE_ORDER_STATUS.RECEIVED}>
                                    Received
                                </option>
                                <option value={PURCHASE_ORDER_STATUS.CANCELLED}>
                                    Cancelled
                                </option>
                            </select>
                            <Button
