        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
               ...
               <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Received Order Summary</h1>
            </div>
            <p className="text-slate-500 font-medium">Completed procurement: {order.po_code}</p>
          </div>
        </div>
        <div className="hidden md:block">
           <span className="px-4 py-2 rounded-full text-sm font-bold bg-emerald-100 text-emerald-800 border border-emerald-200 shadow-sm uppercase tracking-widest">
             Completed & Received
           </span>
        </div>
      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                   <div className="space-y-1">
                      ...
                      <p className="text-base font-bold text-slate-900">{order.supplier?.name || '-'}</p>
                   </div>
                   <div className="space-y-1">
                      <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Warehouse Location</Label>
                      <div className="flex items-center gap-2">
                         <Warehouse className="h-4 w-4 text-emerald-500" />
                         <p className="text-base font-bold text-slate-900">{order.warehouse?.name || '-'}</p>
                      </div>
                   </div>
                </div>
                <div className="space-y-4">
                   <div className="space-y-1">
                      <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Order Date</Label>
                      <div className="flex items-center gap-2">
                         <Calendar className="h-4 w-4 text-slate-400" />
                         <p className="text-base font-bold text-slate-900">
                           {order.order_date ? new Date(order.order_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }) : '-'}
                         </p>
                      </div>
                   </div>
                   <div className="space-y-1">
                      <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Received Date</Label>
                      <div className="flex items-center gap-2">
                         <CheckCircle2 className="h-4 w-4 text-emerald-500" />
                         <p className="text-base font-bold text-emerald-700">
                           {new Date(order.updated_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}
                         </p>
                      </div>
                   </div>
                </div>
                   ...
                   <tbody className="divide-y divide-slate-100">
                      {order.items?.map((item) => (
                         <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="p-4 text-right font-black text-emerald-600">{Number(item.received_quantity).toFixed(2)}</td>
           ...
           <Card className="border-none bg-white shadow-sm">
              ...
              <CardContent className="pt-6 space-y-4">
                 ...
                 <div className="pt-2">
                    ...
                    <div className="bg-emerald-600 rounded-2xl p-6 shadow-lg shadow-emerald-100 flex justify-between items-center">
