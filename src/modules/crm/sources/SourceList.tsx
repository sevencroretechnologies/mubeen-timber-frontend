                  ...
                  items.map((item) => (
                    <div key={item.id} className="flex items-center justify-between gap-3 px-4 py-3">
                      <div className="flex-1 min-w-0">
                        ...
                        {item.source_code ? (
                          <span className="inline-block mt-0.5 text-[11px] bg-blue-50 text-blue-600 border border-blue-100 px-1.5 py-0.5 rounded font-mono">{item.source_code}</span>
                        ) : (
                          <span className="text-xs text-slate-400 italic">No code</span>
                        )}
                      </div>
                      <div className="flex items-center gap-1 shrink-0">
                        <button onClick={() => { setSelectedSource(item); setIsViewOpen(true); }} className="p-1.5 rounded hover:bg-slate-100 text-slate-500 hover:text-blue-600 transition-colors" title="View"><Eye className="h-4 w-4" /></button>
